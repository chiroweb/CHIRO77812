...
export async function GET(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const result = await sql`SELECT * FROM blog_posts WHERE id = ${idNum}`;
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const current = await sql`SELECT title, slug FROM blog_posts WHERE id = ${idNum}`;
    let finalSlug = current.rows[0]?.slug || "";

    if (!current.rows[0] || current.rows[0].title !== title || !finalSlug) {
      const slug = generateSlug(title);
      const existing = await sql`SELECT id FROM blog_posts WHERE slug = ${slug} AND id != ${idNum}`;
...
export async function DELETE(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const result = await sql`DELETE FROM blog_posts WHERE id = ${idNum} RETURNING id`;
